...
func Test_TaskLogs(t *testing.T) {
	...
	var (
		...
		taskLogs = []codersdk.TaskMessage{
			{
				ID:      0,
				Content: "What is 1 + 1?",
				Type:    codersdk.TaskMessageTypeInput,
				Time:    clock,
			},
			{
				ID:      1,
				Content: "2",
				Type:    codersdk.TaskMessageTypeOutput,
	...
	tests := []struct {
		args        []string
		expectLogs  []codersdk.TaskMessage
		expectError string
			...
			handler: func(t *testing.T, ctx context.Context) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
					...
					case fmt.Sprintf("/api/experimental/tasks/me/%s/logs", taskID.String()):
						httpapi.Write(ctx, w, http.StatusOK, taskLogs)
					default:
